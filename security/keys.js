...
const { forComponent } = require('#js/utility/logging/harper_logger');
const envManager = require('#js/utility/environment/environmentManager');
const hdbTerms = require('#src/utility/hdbTerms');
const { CONFIG_PARAMS } = hdbTerms;
const certificatesTerms = require('#js/utility/terms/certificates');
const tls = require('node:tls');
const { relative, join } = require('node:path');
const { CERTIFICATE_VALUES } = certificatesTerms;
const assignCmdenvVars = require('#js/utility/assignCmdEnvVariables');
const configUtils = require('#js/config/configUtils');
const { table, getDatabases, databases } = require('#src/resources/databases');
const logger = forComponent('tls').conditional;
const { getThisNodeName, getThisNodeUrl, urlToNodeName, clearThisNodeName } = require('#src/server/nodeName');
