...
describe('Test lmdbUpdateRecords module', () => {
	...
	describe('Test lmdbUpdateRecords function', () => {
		...
		it('Test updating 1 row', async () => {
			...
			let expected_search = testUtils.assignObjecttoNullObject(update_obj.records[0]);
			expected_search.__createdtime__ = m_time - 1;
			expected_search.__updatedtime__ = m_time;
			expected_search.height = null;

			let results = await testUtils.assertErrorAsync(lmdb_update_records, [update_obj], undefined);
			...
			let height_results = testUtils.assertErrorSync(search_utility.iterateDBI, [transaction, 'height'], undefined);
		...
		it('Test update record with no hash attribute', async () => {
			...
			let update1 = testUtils.deepClone(update_obj);
			await testUtils.assertErrorAsync(lmdb_update_records, [update1], no_hash_error);

			let update2 = testUtils.deepClone(update_obj);
			update2.id = null;
			await testUtils.assertErrorAsync(lmdb_update_records, [update2], no_hash_error);

			let update3 = testUtils.deepClone(update_obj);
			update3.id = undefined;
			await testUtils.assertErrorAsync(lmdb_update_records, [update3], no_hash_error);

			let update4 = testUtils.deepClone(update_obj);
			update4.id = '';
			await testUtils.assertErrorAsync(lmdb_update_records, [update4], no_hash_error);
