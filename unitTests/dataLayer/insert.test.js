...
describe('Test insert module', () => {
	...
	describe('Test upsert method', () => {
		...
		it('NOMINAL - should return upsert response with upserted_hashes value', async () => {
			let results = await testUtils.assertErrorAsync(insert_rw.upsert, [UPSERT_OBJECT_TEST], undefined);
			assert.deepStrictEqual(results, EXPECTED_UPSERT_RESP);
		});

		it('Should return HdbError if operation is not upsert', async () => {
			const upsert_obj = testUtils.deepClone(UPSERT_OBJECT_TEST);
			upsert_obj.operation = 'insert';
			const expected_err = testUtils.generateHDBError('invalid operation, must be upsert', 500);
			await testUtils.assertErrorAsync(insert_rw.upsert, [upsert_obj], expected_err);
		});
		...
		it('Should return HdbError if there is a schema validation error', async () => {
			...
			const expected_err = testUtils.generateHDBError(test_err_msg, 400);
			await testUtils.assertErrorAsync(insert_rw.upsert, [UPSERT_OBJECT_TEST], expected_err);
		});
	...
	describe('Test insertData method', () => {
		const insert_object_test = testUtils.deepClone(UPSERT_OBJECT_TEST);
		...
		it('NOMINAL - should return insert response with inserted_hashes value', async () => {
			...
			const results = await testUtils.assertErrorAsync(insert_rw.insert, [insert_object_test], undefined);
			assert.deepStrictEqual(results, expected_insert_resp);
		});

		it('Should return HdbError if operation is not insert', async () => {
			const insert_obj = testUtils.deepClone(insert_object_test);
			insert_obj.operation = 'upsert';
			await testUtils.assertErrorAsync(insert_rw.insert, [insert_obj], new Error('invalid operation, must be insert'));
		});
		...
		it('Should return HdbError if insertValidator returns error', async () => {
			...
			await testUtils.assertErrorAsync(insert_rw.insert, [insert_obj], expected_err);
		});
		...
		it('Should return HdbError if schema table does not exist', async () => {
			...
			check_schema_stub.returns(test_err_msg);
			const expected_err = testUtils.generateHDBError(test_err_msg, 400);
			await testUtils.assertErrorAsync(insert_rw.insert, [insert_object_test], expected_err);
		});
	});

	describe('Test updateData method', () => {
		const update_object_test = testUtils.deepClone(UPSERT_OBJECT_TEST);
		...
		it('NOMINAL - should return update response with updated_hashes value with existing rows', async () => {
			...
			const results = await testUtils.assertErrorAsync(insert_rw.update, [update_object_test], undefined);
		...
		it('Should return HdbError if operation is not update', async () => {
			const update_obj = testUtils.deepClone(update_object_test);
			update_obj.operation = 'upsert';
			await testUtils.assertErrorAsync(insert_rw.update, [update_obj], new Error('invalid operation, must be update'));
		});

		it('Should return HdbError if insertValidator returns error', async () => {
			const update_obj = testUtils.deepClone(update_object_test);
			update_obj.schema = '';
			const expected_err = testUtils.generateHDBError("'schema' is not allowed to be empty", 400);
			await testUtils.assertErrorAsync(insert_rw.update, [update_obj], expected_err);
		});
		...
		it('Should return HdbError if schema table does not exist', async () => {
			...
			check_schema_stub.returns(test_err_msg);
			const expected_err = testUtils.generateHDBError(test_err_msg, 400);
			await testUtils.assertErrorAsync(insert_rw.update, [update_object_test], expected_err);
		});
	});
});
